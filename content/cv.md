---
title: "CV"
description: "Generative AI · 3D & XR · Computer Vision Research Engineer — Paris, France"
featured_image: "/img/projects/alienor/fMyHGJ.jpg"
showTableOfContents: true
---

<p class="mb4">
  <a href="/pdf/Maelys_2026_pro.pdf" download class="link">📄 Download CV (PDF)</a>
</p>

<object data="/pdf/Maelys_2026_pro.pdf" type="application/pdf" width="100%" height="800" style="border: 1px solid #5A1818; border-radius: 6px; min-height: 70vh;" title="CV Maëlys Jusseaux">
  <p>Your browser does not support PDF embedding. <a href="/pdf/Maelys_2026_pro.pdf">Download the CV (PDF)</a>.</p>
</object>

---

## Experience

### 3D & Computer Vision Research Engineer
**Nfinite SAS** · Paris · `Jul 2022 – Present`

- AI engineering, development and project management (multimodal generative AI, fine-tuning, data management, AI pipelines)
- Development and maintenance of AI pipeline tools (automated training, custom ComfyUI nodes, Gradio playgrounds)
- Research, development, evaluation and deployment of AI models (technology watch, reports)
- R&D of 3D features and 3D scanning pipelines
- Development and maintenance of 3D model and image management tools
- Development and maintenance of data management tools
- Documentation of developed features and writing of scientific/technical reports

### Co-founder & Secretary
**Cosmosia Association** · Paris · `Jul 2022 – Present`

- Organisation of scientific and artistic events between research-creation laboratories and French cultural institutions, at the intersection of art and technology

### Researcher, TD Pipeline
**Opuscope SAS** · Paris · `Sept 2017 – Apr 2022`

- R&D for Minsar and Wanderland applications (XR)
- Pipeline coordination between marketing, sales and production departments
- TD tooling, pipeline, technical documentation and tutorials
- 3D, XR and video creation and production
- Technical support and client assistance
- Masterclasses, workshops, conferences
- Research and writing of scientific reports and articles (thesis, symposiums)

### 3D Generalist
**Iconem** · Paris · `May – Jul 2017`

- R&D for cultural heritage digitisation and visualisation
- Post-production of 3D scans, creation and production of 3D content
- TD tooling and technical documentation

### 3D Generalist & Developer
**Association André Breton** · Paris · `May 2016 – May 2017`

- Post-production of 3D scans
- Development of a 3D scan visualisation application for the **Centre Pompidou** (still on display today)

---

## Education

### PhD — Esthétique, Sciences et Technologies des Arts
**Paris 8** · 2018–2021

Thesis: *The Numinous Heritage Experience* — eliciting transcendence through immersive technologies and cultural heritage.

### Master — Arts et Technologies de l'Image
**Paris 8** · 2016–2018

### Licence — Arts et Technologies de l'Image
**Paris 8** · 2015–2016

### Master — Médiation Culturelle, Patrimoine et Numérique
**Paris 8 / Paris 10** · 2013–2014

### Master — Muséologie
**École du Louvre** · 2012–2013

### Licence — Histoire de l'Art et Archéologie
**Paris 4 (Sorbonne)** · 2009–2012

---

## Skills

### AI Development

**Frameworks & Tooling** · PyTorch · TensorFlow · Keras · ComfyUI · Kohya-SS · Gradio · Diffusers · Transformers

**Image & Video Generation** · Stable Diffusion · Flux · Wan 2.2 · Bytedance Seedream API · LoRAs · BLoRAs · IPAdapters · Condeltas

**Text Generation** · Open-webui · Gemini API · OpenAI API · Florence · Moondream · Deepseek

**Voice Generation** · Tortoise-TTS · Zonos · Index-TTS2 · Coqui XTTS2

**3D Generation & Segmentation** · Hunyuan · Trellis · Tripo · Partpacker · Craftsman3D · SAMesh · HunyuanPart3D · Omnipart · PartField · SAMPart3D

### Programming

**Languages (Mid)** · Python · LaTeX · Markdown

**Languages (Junior)** · C++ · MEL · Lua · Java · JavaScript

**Platforms** · Linux · Windows · MacOS

**Versioning & Development** · Git · Docker · Conda · UV

### Digital Creation

**3D** · Blender · Maya · ZBrush · Mudbox · 3DCoat

**Scan** · RealityCapture · Metashape · Meshroom · LumaAI

**2D** · Photoshop · Krita · Substance Suite · AwesomeBump · XNormal

**Video** · Premiere · After Effects · Kdenlive · Olive Editor

### XR Development

**Engines** · Godot · Unity · Unreal Engine

**Hardware** · HoloLens · MagicLeap · Meta Quest/Rift · HTC Vive/Vive Pro · WMR (Lenovo, Dell)
